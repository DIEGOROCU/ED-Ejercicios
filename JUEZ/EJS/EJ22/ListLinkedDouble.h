...
public:
    ...
    void advance(reverse_iterator &it, int n) {
        for (int i = 0; i < n && it.current != head; ++i) {
            ++it;
        }
    }

    int elem(reverse_iterator &it) const {
        return *it;
    }
...
void ListLinkedDouble::unzip(ListLinkedDouble &other)
{
    ...
    {
        // Guardamos los nodos siguientes (posiciones pares)
        Node *siguiente = current->next;
        // Actualizamos el nodo actual (despues no tendremos acceso al siguiente)
        current = siguiente->next;

        // Quitamos el nodo en posicion par de la lista original
        detach(siguiente);
        // Insertamos el nodo en la otra lista, al final
        other.attach(siguiente, other.head);
    }
}
