...
public:
  ...
  std::pair<bool, T> upper_bound(const T &busc) const
  {
    return upper_bound(root_node, busc);
  }

  T minimun() const
  {
    assert(root_node != nullptr);
    return minimun(root_node);
  }
  ...
  void erase_min()
  {
    ...
    {
      auto [new_root, removed] = erase(root_node, minimun(root_node));
      root_node = new_root;
      if (removed)
      {
        num_elems--;
...
private:
  ...
  std::pair<bool, T> upper_bound(Node *root, const T &busc) const
  {
    if (root == nullptr) {
        return {false, T()}; // No se encontró un elemento válido
    } else if (root->elem >= busc) {
        // Si el elemento actual es mayor que el buscado, seguimos buscando en el subárbol izquierdo
        return upper_bound(root->left, busc);
    } else {
        // Si el elemento actual es menor o igual al buscado, puede ser una solución válida
        auto [found, candidate] = upper_bound(root->right, busc);
        if (found) {
            return {true, candidate}; // Si encontramos un mejor candidato en el subárbol derecho, lo devolvemos
        } else {
            return {true, root->elem}; // Si no, el elemento actual es el mejor candidato
        }
    }
  }

  T minimun(Node *root) const
  {
