...
public:
  ...
  void desentramar(ListLinkedDouble & otro){
    Node* actual = head->next->next;
    while (actual != head && actual->prev != head){
      actual->prev->next = actual->next;
      actual->next->prev = actual->prev;
      Node* sig = actual->next;
      actual->prev = otro.head->prev;
      actual->next = otro.head;
      otro.head->prev->next = actual;
      otro.head->prev = actual;
      actual = sig;
      actual = actual->next;
    }
  }
