...
public:
  ...
  ListLinkedDouble desDuplicar(){
    ListLinkedDouble dev;
    Node* actual = this->head;
    Node* actualDev = dev.head;
    while (actual->next != this->head){
        if (actual->next->value == actual->value){
            Node* sig = actual->next->next;
            // Actualizamos el nodo siguiente
            actual->next->prev = actualDev;
            actual->next->next = dev.head;
            // Actual de la de devolver
            actualDev->next = actual->next;
            actualDev = actual->next;
            //
            actual->next = sig;
        }
        else {
