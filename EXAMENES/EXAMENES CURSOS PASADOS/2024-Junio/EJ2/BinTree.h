...
public:
  ...
  std::tuple<int, int, int, int> seccionMayor()
  {
    if (empty())
    {
      return {0, 0, 0, 0};
    ...
    {
      auto [izNum, izCan, izMaxNum, izMaxCan] = left().seccionMayor();
      auto [drNum, drCan, drMaxNum, drMaxCan] = right().seccionMayor();

      int numDev = root(), numCan = 1, numMaxDev, numMaxCan;

      numCan += (izCan) * (izNum == numDev);
      numCan += (drCan) * (drNum == numDev);

      if (numCan >= izMaxCan && numCan >= drMaxCan){
        numMaxDev = numDev;
        numMaxCan = numCan;
        return {numDev, numCan, numMaxDev, numMaxCan};
      }
      else if (izMaxCan >= drMaxCan){
        numMaxDev = izMaxNum;
        numMaxCan = izMaxCan;
        return {numDev, numCan, numMaxDev, numMaxCan};
      }
