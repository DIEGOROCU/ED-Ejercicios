...
public:
  ...
  void interseccion(ListLinkedDouble const& otra){
    Node* actual = head->next;
    Node* actualOtra = otra.head->next;
    while (actual != head && actualOtra != otra.head){
      if (actual->value < actualOtra->value) {
      ...
      else if (actual->value > actualOtra->value) {
        actualOtra = actualOtra->next;
      }
      else {
        actual = actual->next;
        actualOtra = actualOtra->next;
